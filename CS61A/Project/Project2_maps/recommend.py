...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        min_centroid = find_closest(restaurant_location(restaurant),centroids)
        restaurant_centroid_pairs.append([min_centroid,restaurant])
...
def find_centroid(restaurants):
    ...
    mean_x = mean([restaurant_location(restaurant)[0] for restaurant in restaurants])
    mean_y = mean([restaurant_location(restaurant)[1] for restaurant in restaurants])
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        #step1: assign restaturant to centroids
        grouped_locations=group_by_centroid(restaurants,old_centroids)
        #step2: recompute centroids
        centroids = [find_centroid(grouped_location) for grouped_location in grouped_locations]
        n += 1
    return centroids
...
def best_predictor(user, restaurants, feature_fns):
    ...
    "*** YOUR CODE HERE ***"
    p_to_rsquare = {find_predictor(user,reviewed,f)[0]: find_predictor(user,reviewed,f)[1] for f in feature_fns}
    return max(p_to_rsquare.keys(),key=lambda key:p_to_rsquare[key])
...
def rate_all(user, restaurants, feature_functions):
    ...
    d_reviewed = {restaurant_name(r): user_rating(user,restaurant_name(r)) for r in restaurants.values() if r in reviewed.values()}
    d_pred = {restaurant_name(r): predictor(r) for r in restaurants.values() if r not in reviewed.values()}
    return dict(list(d_reviewed.items())+list(d_pred.items()))
