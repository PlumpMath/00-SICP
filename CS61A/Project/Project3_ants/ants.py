    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if not self.ant.container:
                self.ant = None
            elif self.ant.container and self.ant.ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while (nearest_place is not None and nearest_place != hive):
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        Insect.reduce_armor(self, amount) #if reduce_armor perform first, then self is removed from place immediately
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        distance = 0
        while (nearest_place is not None and nearest_place != hive):
            if nearest_place.bees and distance <= self.max_range:
                return random_or_none(nearest_place.bees)
            distance += 1
            nearest_place = nearest_place.entrance
        return None
...
class QueenAnt(ScubaThrower):
    ...
    def action(self, colony):
        ...
        backword = self.place
        while backword.exit is not None:
            backword = backword.exit
            double_damage(backword)
